  ...
  try {
    ...
    querySnapshot.forEach(async (doc) => {
      let form =
        ...
        doc.data().data_hora.nanoseconds / 1_000_000;
      let horaedata = new Date(form);
      registros.push({
        id: doc.id,
        ...doc.data(),
        data_hora: horaedata.toLocaleString("pt-BR"),
      });
    });
...
const disableComanda = async (req, res) => {
  ...
  try {
    ...
    if (docSnap.exists() && docSnap.data().ativo) {
      ...
      console.error(
        "Erro ao tentar desativar comanda: comanda nao existe ou nao esta ativada"
      );
      res.json({
        status: "400",
        message: "Comanda nao existe ou nao esta ativada.",
      });
    }
  } catch (err) {
    console.error(err);
