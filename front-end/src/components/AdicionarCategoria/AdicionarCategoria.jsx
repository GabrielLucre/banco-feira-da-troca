...
export default function AdicionarCategoria({
            ...
            if (!exists) {
                try {
                    if (subcategorias.length !== valor.length) {
                        console.log(valor)
                        console.error(`Os arrays subcategorias e valor devem ter o mesmo comprimento: ${subcategorias.length} - ${valor.length}`);
                        setOpenAlertDuplicate(true)
                        return;
                    }
    ...
    const handleAddCategoria = (event) => {
        ...
        setCategoria(value.value.toUpperCase());
    };
    ...
    return (
        <div>
            ...
            >
                ...
                <div onClick={(e) => e.stopPropagation()}>
                    <Alerta state={openAlertSubcategorias} onClose={handleCloseBackdropButton} text="Você não pode apagar porque tem subcategorias/produtos dentro desta categoria!" severity="error" />
                </div>
            </Backdrop>
